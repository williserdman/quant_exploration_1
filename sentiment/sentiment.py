...
(feature_train, target_train), (x_test, y_test) = data.load_data(
    num_words=88000
)  # only 10k most frequent words #this line splits it equally#changed to 88k

# print(feature_train[0])#prints reviews where a integer points to a word

word_index = (
    data.get_word_index()
)  # gives tuples that have the string and the word in them
word_index = {k: (v + 3) for k, v in word_index.items()}
word_index["<PAD>"] = (
    0  # add padding to make every review be the same length with the hopes that it will be ignored
)
word_index["<START>"] = 1
word_index["<UNK>"] = 2
word_index["<UNUSED>"] = 3
...
)  # swaps values and keys, have integer pointing toward word instead of vice-versa

# Making the lenght of every review the same, can do this manually if you want (I didn't)
feature_train = keras.preprocessing.sequence.pad_sequences(
    feature_train, value=word_index["<PAD>"], padding="post", maxlen=256
)
x_test = keras.preprocessing.sequence.pad_sequences(
    x_test, value=word_index["<PAD>"], padding="post", maxlen=256
)
...
def decoder(text):
    ...
    )  # the "?" in get is a default values trying to prevent crashes. the space before .join puts a space between words
...
def review_encode(in_string):
    ...
    for word in in_string:
        try:
            ...
            )  # tries adding the word by throwing it in the dictionary, if there's an error:
        except:
...
with open("review.txt", "r") as f:  # use "relative path"
    for (
        ...
        f.readlines()
    ):  # only one line (text wrapped, but makes easy if there are more reviews)
        ...
        nline = (
            ...
            .strip()
            .split(" ")
        )  # removes random characters, prob better way to do this
        encoded = review_encode(nline)
        encoded = keras.preprocessing.sequence.pad_sequences(
            [encoded], value=word_index["<PAD>"], padding="post", maxlen=256
        )
